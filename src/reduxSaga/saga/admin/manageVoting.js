...
import { get_vote, post_vote } from "../../user/api";
import { GET_VOTE_ERROR, GET_VOTE_SUCCESS, POST_VOTE_ERROR, POST_VOTE_SUCCESS } from "../../user/action";
...
function* handle_get_vote(action) {
    ...
    try {
        ...
        if (status === 200) {
            yield put({ type: GET_VOTE_SUCCESS, payload: data.data });
        }
    } catch (error) {
        yield put({ type: GET_VOTE_ERROR, payload: error });
    }
}

function* handle_post_vote(action) {
    console.log(action, "manageee");
    try {
        let { data } = yield call(post_vote, action);
        yield put({ type: POST_VOTE_SUCCESS, payload:data})
    } catch (error) {
        yield put({ type: POST_VOTE_ERROR, payload: error })
    }
}

export { handle_get_party, handle_post_party, handle_delete_party, handle_get_voter, handle_post_voter, handle_delete_voter, handle_get_election, handle_post_election, handle_delete_election, handle_get_connection, handle_post_connection, handle_delete_connection, handle_get_vote, handle_post_vote }